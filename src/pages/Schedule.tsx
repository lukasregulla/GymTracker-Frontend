...
export default function Schedule() {
  ...
  const getSessionMeta = (session: {
    ...
    const time = formatSessionTime(session.scheduledStartTime);
    ...
    <div className="px-4 pt-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() =>
            navigate(`/log?date=${format(selectedDay, "yyyy-MM-dd")}`)
          }
      ...
      <div className="flex gap-1 bg-surface border border-border rounded-xl p-2">
        {weekDays.map((day) => {
          ...
          return (
            ...
            >
              <span
                className={`text-[10px] font-medium ${isSelected ? "text-white" : "text-text-secondary"}`}
              >
                {format(day, "EEE")[0]}
              </span>
              <span
                className={`text-sm font-bold ${isSelected ? "text-white" : isToday ? "text-primary" : "text-white"}`}
              >
                {format(day, "d")}
              </span>
              <div
